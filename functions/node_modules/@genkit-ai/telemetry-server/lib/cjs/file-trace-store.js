...
exports.Index = Index;
function isFilterMatch(d, filter) {
    if (!filter)
        return true;
    ...
    if (gte) {
        for (const k of Object.keys(gte)) {
            const val = d[k];
            if (typeof val !== 'number' || val < gte[k])
                return false;
        }
    }
    if (lt) {
    ...
    if (lte) {
        for (const k of Object.keys(lte)) {
            const val = d[k];
            if (typeof val !== 'number' || val > lte[k])
                return false;
        }
    }
    if (contains) {
        for (const k of Object.keys(contains)) {
            ...
            const match = Array.isArray(filterVal)
                ? filterVal.some((fv) => val.toLowerCase().includes(`${fv}`.toLowerCase()))
