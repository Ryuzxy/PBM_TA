...
class ProcessManager {
    ...
    start(options) {
        utils_1.logger.debug(`Starting process: ${this.command} ${this.args.join(' ')}`);
        return new Promise((resolve, reject) => {
            ...
            if (!options?.nonInteractive) {
                ...
                this.appProcess.stderr?.on('data', (data) => {
                    utils_1.logger.debug(`[ProcessManager Stderr] ${data.toString()}`);
                });
                this.appProcess.stdout?.on('data', (data) => {
                    utils_1.logger.debug(`[ProcessManager Stdout] ${data.toString()}`);
                });
            }
