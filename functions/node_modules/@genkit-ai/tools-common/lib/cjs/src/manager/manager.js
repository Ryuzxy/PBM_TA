...
exports.RuntimeManager = exports.BaseRuntimeManager = exports.GENKIT_REFLECTION_API_SPEC_VERSION = void 0;
...
class BaseRuntimeManager {
    ...
    constructor(telemetryServerUrl, projectRoot, processManager, disableRealtimeTelemetry = false) {
        this.telemetryServerUrl = telemetryServerUrl;
        this.projectRoot = projectRoot;
        this.processManager = processManager;
        this.disableRealtimeTelemetry = disableRealtimeTelemetry;
    }
    ...
    async listLogs(input) {
        ...
        const response = await axios_1.default
            .get(fullUrl)
            .catch((err) => this.httpErrorHandler(err, `Error listing logs for url='${fullUrl}'.`));
    ...
    async streamTrace(input, streamingCallback) {
        ...
        return new Promise((resolve, reject) => {
            stream.on('data', (chunk) => {
                ...
                while (buffer.includes('\n\n')) {
                    ...
                    try {
                        ...
                        logger_1.logger.error(`Error parsing stream data: ${err}`);
                    }
                }
            });
            stream.on('end', () => {
            ...
            stream.on('error', (err) => {
                logger_1.logger.error(`Stream error for traceId='${traceId}': ${err}`);
                reject(err);
            });
        });
    }
...
exports.BaseRuntimeManager = BaseRuntimeManager;
class RuntimeManager extends BaseRuntimeManager {
    manageHealth;
    ...
    static async create(options) {
        if (options.experimentalReflectionV2) {
            const { RuntimeManagerV2 } = await Promise.resolve().then(() => __importStar(require('./manager-v2')));
    ...
    async listValues(input) {
        ...
        try {
            const response = await axios_1.default.get(`${runtime.reflectionServerUrl}/api/values`, {
                params: {
                    type: input.type,
                },
            });
            ...
            if (err.response?.status === 404) {
                ...
                throw new types_1.GenkitToolsError(`Bad request: ${err.response?.data}`);
            }
            this.httpErrorHandler(err, 'Error listing values.');
        }
    }
            ...
            return new Promise((resolve, reject) => {
                ...
                response.data.on('end', () => {
                    try {
                        ...
                        const parsed = action_1.RunActionResponseSchema.parse(responseData);
                        if (response.headers['x-genkit-version']) {
                        ...
                        reject(new types_1.GenkitToolsError(`Failed to parse response: ${err}`));
                    }
                });
    ...
    async cancelAction(input) {
        ...
        try {
            const response = await axios_1.default.post(`${runtime.reflectionServerUrl}/api/cancelAction`, { traceId: input.traceId }, {
                headers: {
                    'Content-Type': 'application/json',
                },
            });
            ...
            if (axiosError.response?.status === 404) {
                const error = new types_1.GenkitToolsError('Action not found or already completed');
                error.data = {
    ...
    async setupRuntimesWatcher() {
        try {
            ...
            logger_1.logger.debug(`Watching runtimes in ${runtimesDir}`);
