...
async function runNewEvaluation(manager, request) {
    ...
    if (datasetId) {
        ...
        utils_1.logger.debug(`Fetching dataset ${datasetId}...`);
        ...
        inferenceDataset = rawData;
    }
    utils_1.logger.debug('Running inference...');
...
async function runEvaluation(params) {
    ...
    const scores = {};
    utils_1.logger.debug('Running evaluation...');
    ...
    for (const action of evaluatorActions) {
        ...
        utils_1.logger.debug(`Finished evaluator '${action.name}'`);
    ...
    utils_1.logger.debug('Finished evaluation, writing key...');
...
async function bulkRunAction(params) {
    ...
    for (const sample of fullInferenceDataset) {
        utils_1.logger.debug(`Running inference '${actionRef}' ...`);
        if (actionType === 'model') {
    ...
    utils_1.logger.debug(`Gathering evalInputs...`);
...
async function runPromptAction(params) {
    ...
    try {
        ...
        renderedPrompt = {
            ...
            result: runActionResponse.result,
        };
    }
    catch (e) {
