...
exports.start = new commander_1.Command('start')
    ...
    .option('--disable-realtime-telemetry', 'Disable real-time telemetry streaming')
    .option('--cors-origin <origin>', 'specify the allowed origin for CORS requests')
    .option('--experimental-reflection-v2', 'start the experimental reflection server (WebSocket)')
    .option('--write-env-file <file>', 'write environment variables in .env format to the provided file')
    ...
    const devEnv = await (0, manager_utils_1.getDevEnvVars)(projectRoot, {
    ...
    if (options.writeEnvFile) {
        const content = Object.entries(envVars)
            .map(([k, v]) => `${k}=${v}`)
            .join('\n');
        fs_1.default.writeFileSync(options.writeEnvFile, content);
        utils_1.logger.info(`Wrote environment variables to ${options.writeEnvFile}`);
    }
    let manager;
    let processPromise;
    if (exports.start.args.length > 0) {
        const result = await (0, manager_utils_1.startDevProcessManager)(projectRoot, exports.start.args[0], exports.start.args.slice(1), {
            disableRealtimeTelemetry: options.disableRealtimeTelemetry,
            corsOrigin: options.corsOrigin,
            experimentalReflectionV2: options.experimentalReflectionV2,
            envVars,
        ...
        manager = await (0, manager_utils_1.startManager)({
            projectRoot,
            manageHealth: true,
            corsOrigin: options.corsOrigin,
            experimentalReflectionV2: options.experimentalReflectionV2,
